...
export async function GET(req: NextRequest) {
  ...
  const stateCookie = req.cookies.get(CLICKUP_OAUTH_STATE_COOKIE)?.value;
  const [expectedState, rawReturnTo] = stateCookie ? stateCookie.split(':', 2) : [undefined, '/'];
  ...
  if (!expectedState || !state || state !== expectedState) {
    logger.warn({ hasCookie: Boolean(expectedState), hasState: Boolean(state) }, 'ClickUp OAuth state mismatch');
